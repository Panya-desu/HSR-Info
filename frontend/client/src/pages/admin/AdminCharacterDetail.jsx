...
export default function AdminCharacterDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const charRes = await fetch(`${apiUrl}/admin/characters/${name}`, {
  ...
  return (
    <div className="detail-page admin-detail">
      <div className="admin-detail-header">
        <Link to="/admin/characters" className="nav-btn list-btn">
          <ChevronLeft size={20} /> Back to List
        </Link>
        <div className="admin-header-actions">
          <Link to={`/admin/characters/${slugify(character.name)}/edit`} className="edit-btn-large">
            <Edit2 size={20} /> Edit Character
          </Link>
        </div>
      </div>
